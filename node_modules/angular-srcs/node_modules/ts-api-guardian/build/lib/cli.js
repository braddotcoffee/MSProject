    ...
    if (mode === 'help') {
        ...
        if (mode === 'out') {
            for (var _b = 0; _b < targets.length; _b++) {
                var _c = targets[_b], entrypoint = _c.entrypoint, goldenFile = _c.goldenFile;
                main_1.generateGoldenFile(entrypoint, goldenFile, options);
            }
        }
        else {
            var hasDiff = false;
            for (var _d = 0; _d < targets.length; _d++) {
                var _e = targets[_d], entrypoint = _e.entrypoint, goldenFile = _e.goldenFile;
                ...
                if (diff) {
                    ...
                    for (var _f = 0; _f < lines.length; _f++) {
                        var line = lines[_f];
